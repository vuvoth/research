...
mod aggregation {
    ...
    impl AggregationCircuit {
        ...
        pub fn accumulator_indices() -> Vec<(usize, usize)> {
            (0..4 * LIMBS * 3).map(|idx| (0, idx)).collect()
        }
...
fn gen_srs(k: u32) -> ParamsKZG<Bn256> {
    ParamsKZG::<Bn256>::read_or_create_srs(k, OsRng)
}
...
fn gen_application_snark(params: &ParamsKZG<Bn256>) -> aggregation::Snark {
    let circuit = gen_merkle_circuit_data(4);
    let pk = gen_pk(params, &circuit);
    let instances = vec![vec![circuit.hash_root]];
